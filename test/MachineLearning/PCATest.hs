...
getDimReducerSmokeTest =
  let nFeatures = 4
      ...
      m10 = m * 10
      (reduceDims, _, mReduced) = getDimReducer m 2
...
getDimReducer_rvSmokeTest =
  let nFeatures = 4
      nExamples = 7
      m = LA.matrix nFeatures [1 .. fromIntegral $ nFeatures*nExamples]
      m10 = m * 10
      (reduceDims, k, mReduced) = getDimReducer_rv m 0.5
      m10Reduced = reduceDims m10
  in do
...
tests = [ testGroup "smoke test" [
            ...
            , testCase "getDimReducer_rv" getDimReducer_rvSmokeTest
            ]
        ]
